...
public class HTMLParser {
    ...
    */
    ...
    public var root : HTMLNode? {
        return rootNode
    }
    
    public init(data:NSData, encoding: NSStringEncoding, option: CInt, inout error: NSError?) {
        if data.length > 0 {
            ...
            _doc = htmlReadDoc(UnsafePointer<CUnsignedChar>(data.bytes), url, enc, optionHtml)
    ...
    */
    public init(html: String, encoding: NSStringEncoding, option: CInt, inout error: NSError?) {
    ...
    public convenience init(html: String, encoding : NSStringEncoding, inout error: NSError?) {
