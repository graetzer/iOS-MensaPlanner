          ...
          for node in items! where node.kind() == .xmlElementKind {
            ...
            if let type = el.attribute(forName: "class")?.stringValue() {
              if type.contains("menue-category") {
                ...
                let title = self.textFromElement(el: el)
                if title.starts(with: "+") {
                  menu.title = String(title[title.index(after: title.startIndex)...])
                } else {
                  menu.title = title
                }
  ...
  private func textFromElement(el : GDataXMLElement) -> String {
    ...
    for node in el.children() {
      if node.kind() == .xmlTextKind {
        ...
        if let child = node as? GDataXMLElement, child.name() == "span" {
          buffer += textFromElement(el: child)
        }
      }
    }
